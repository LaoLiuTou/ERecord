package com.erecord.controller.user;
...
import com.erecord.service.user.IUserService;
import com.erecord.utils.MD5Encryption;
import com.erecord.utils.TokenUtils;
import com.erecord.utils.TokenUtils.TokenBean;
...
public class UserController {
	...
	public Map list(HttpServletRequest request, HttpServletResponse response,User user)
		...
		try {
			...
			if(page!=null&&size!=null){
				...
				paramMap.put("unit_id",user.getUnit_id());
				paramMap.put("cp_id",user.getCp_id());
	...
	@ResponseBody
	public Map mlogin(HttpServletRequest request,User user){
		Map resultMap=new HashMap();
		try {
			if(user.getUsername()!=null&&user.getPassword()!=null){
				...
				if(list.size()>0){
					if(list.get(0).getPassword().equals(MD5Encryption.getEncryption(user.getPassword()).toLowerCase())){
						...
						if(list.get(0).getCp_id()!=null){
							...
							List<Checkpoint> cpList=iCheckpointService.selectCheckpointByParam(paramMap);
							List<Checkpoint> resultCpList = new ArrayList();
							for(Checkpoint cp :cpList){
								if(list.get(0).getCp_id().contains(cp.getId()+"")){
									resultCpList.add(cp);
								}
							}
							resultMap.put("cp", resultCpList);
