...
public class CORSFilter implements Filter { 
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
		...
		if(uri.startsWith("/filesUpload")||uri.startsWith("/filesDelete")||uri.startsWith("/upload")||
				uri.startsWith("/MP_verify_vc2JUff3wbbpReuT.txt")||uri.startsWith("/exportRecords")  ){
			chain.doFilter(req, res);  
		}
		
		else{
			
		    if( !uri.startsWith("/login")&&!uri.startsWith("/mlogin")&&!uri.startsWith("/pages")){
